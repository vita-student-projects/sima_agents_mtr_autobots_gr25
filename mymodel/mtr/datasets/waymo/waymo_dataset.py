...
class WaymoDataset(DatasetTemplate):
    ...
    def generate_centered_trajs_for_agents(self, center_objects, obj_trajs_past, obj_types, center_indices, sdc_index, timestamps, obj_trajs_future):
        ...
        ret_obj_trajs = torch.cat((
            obj_trajs[:, :, :, 0:6],  # 6 attributes: cx, cy, cz, dx, dy, dz
            object_onehot_mask,  # 5 attributes: one-hot encoding mask for object types
            object_time_embedding, # num_timestamps + 1 attributes: 
            object_heading_embedding, # 2 attributes: sin(heading), cos(heading)
            obj_trajs[:, :, :, 7:9], # 2 attributes: vel_x, vel_y
            acce,  # 2 attributes: acceleration in x and y directions
